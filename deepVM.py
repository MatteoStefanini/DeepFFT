...
class DeepMix(nn.Module):
    def __init__(self, channels=16, window=128, num_classes=2):
        ...
        #self.fc1 = nn.Linear(1024, 16).cuda()
        #self.fc2 = nn.Linear(16, num_classes).cuda()
...
def SaveMeanStd():
    ...
    df.append(pd.read_csv("VMdata/WEB1PRO.data", sep='\t', usecols=interestingColumns))  # , nrows=chunks))
    df.append(pd.read_csv("VMdata/WEB2PRO.data", sep='\t', usecols=interestingColumns))  # , nrows=chunks))
    df.append(pd.read_csv("VMdata/WEB3PRO.data", sep='\t', usecols=interestingColumns))  # , nrows=chunks))
